# Explainer · Správa SPFx: App Catalog, API access a tenant-wide extensions

Doplněk k [`README.md`](README.md), který řeší **vývoj** SPFx. Tenhle explainer je
o druhé polovině životního cyklu: co s hotovým `.sppkg` dělá **správce tenantu** — kam
ho nasadit, jaká oprávnění mu schválit, jak zjistit, co všechno v tenantu běží, a proč
je API access nejnebezpečnější dialog v celém SharePoint admin centru.
...
```powershell
Add-PnPApp -Path .\reseni.sppkg -Scope Tenant -Publish
...
lze vypnout jedním přepínačem (`Disabled`).
...
> přesouvá; podporované scénáře isolated web parts a verze SPFx se mění. Před během
> proklikat cestu v portálu.
