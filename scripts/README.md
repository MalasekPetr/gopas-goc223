...
> Repo je public. **Žádný skript nebude mít zabudované identifikátory** — tenant GUID (globálně unikátní identifikátor), ClientId
...
Cílový tenant: tenant Microsoft 365 (M365) Developer `cloudedu.cz` (viz [`../environment.md`](../environment.md)).
Účty `jmeno.prijmeni@cloudedu.cz` (bez diakritiky, max. 25), licence E5 Developer (nejvyšší plán, vývojářská varianta), role
...
|---|---|---|---|
| 1. Účty studentů (vytvoření/reaktivace, E5 licence, role Global administrator, GA) | `New-CourseStudents.ps1` `[PLÁNOVANÉ]` | Graph | `day-1/onboarding` |
| 2. Studentské weby — **jen fallback** (studenti si `-dev/-test/-prod` weby vytváří sami v D1 labu) + seedování úmyslného driftu a demo obsahu do nich | `New-CourseStudentSites.ps1` `[PLÁNOVANÉ]` | PnP | `day-2/powershell-deep-dive`, `day-3/staging-environments` |
...
| 4. Azure prostředky per student (Storage/Blob **gen-purpose v2**, Function App **Flex Consumption**, Event Grid) + **DCR per student** proti sdílenému Log Analytics workspace | `New-CourseStudentAzureResources.ps1` `[PLÁNOVANÉ]` | Az/ARM | `day-4/siem-blob-integration` |
...
1. **Interactive** (default) — popup prohlížeče nebo Web Account Manageru (WAM). Pozor: popup jde do default browseru;
