...
| Cert v `Cert:\CurrentUser\My` je, ale `HasPrivateKey` = `False` | ve store je jen veřejná část (import `.cer` místo párování s klíčem; u Hardware Security Module, HSM, je to normální stav), nebo cert z čipové karty bez spárování | přegenerovat pár (`New-SelfSignedCertificate`), u smart card / HSM ověřit minidriver a propagaci certifikátu |
